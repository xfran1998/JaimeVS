...
function run_code(socket, room_name){
    ...
    if (process.platform === 'linux') {
        ...
        if (process.arch === 'arm64') {
            ...
            var child = run_script(path.join(__dirname, 'Processing', 'arm64', 'processing-java.exe'), ["--force", `--sketch=${path.join(__dirname, 'temp', room_name)}`, `--output=${path.join(__dirname, 'temp', room_name,'out')}`, "--run"], {cwd:`${path.join(__dirname, 'temp', room_name)}`}, time_out, function(buf) {
                socket.emit('processing_output_server', buf);
            });
        }
        else{
